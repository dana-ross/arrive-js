window.Arrive = function () {
	"use strict";
	function a(a) {
		return "object" == typeof a && "nodeType"in a && 1 === a.nodeType
	}

	function b() {
		var b;
		for (b in f)f.hasOwnProperty(b) && (a(f[b].target) ? c(f[b]) : d(f[b]));
		g = !1
	}

	function c(a) {
		var b, c = window.innerHeight || document.documentElement.clientHeight;
	...
	function d(a) {
		var b, c, d, e = window.innerHeight || document.documentElement.clientHeight;
	...
	return e.reset = function () {
		...
		g || requestAnimationFrame(b), g = !0
	}), e.register_selector = function (a, b, c) {
		if ("string" != typeof a)throw"selector must be a string";
		return this._register(a, b, c)
	}, e.register_element = function (b, c, d) {
		if (!a(b))throw"element is not a DOM Element object";
		return this._register(b, c, d)
	}, e._register = function (a, b, c) {
		var d = {target: a};
		return "undefined" != typeof b && (d.visible_callback = b), "undefined" != typeof c && (d.no_longer_visible_callback = c), f.push(d), d
	}, e
}(), function () {
